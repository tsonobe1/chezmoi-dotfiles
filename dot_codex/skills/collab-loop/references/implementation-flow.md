...
テストケースを提案または実装する前に、ユーザに「collab-loop に準拠しているか」と聞かれなくても自己点検する。最低限、`作業前提 / 今回の製品機能 / 非対象 / 対象機能 / 観点カテゴリ / 個別振る舞い / 既存テストへの影響` が埋まっているか、各 `it.todo` が条件 + 結果になっているか、user-visible wording contract に反していないか、runtime boundary の evidence plan があるかを確認する。不足があれば実装前に補う。
...
- 1 test = 1 behavior
- 実装詳細より外部から観測できる振る舞いを検証する
- 不要な mocking は行わない
- interaction verification より state verification を優先する
- テスト名は日本語で、条件 + 結果を表現する
- 信頼できていない比較元を前提に比較テストを書かない
