...
(defconst my/org-storypoint-safety-options
  '(("普通" . 0)
    ("少し安全" . 1)
    ("かなり安全" . 2))
  "Safety margins available for three-point Storypoint estimates.")

(defun my/org--format-storypoint (point)
  "Format POINT without trailing .0 when it is an integer."
  (if (= point (floor point))
      (number-to-string (floor point))
    (format "%.1f" point)))

(defun my/org--completing-read-in-order (prompt collection)
  "Read from COLLECTION with PROMPT while preserving collection order."
  (let ((vertico-sort-function nil))
    (completing-read prompt collection nil t)))

(defun my/org--read-storypoint (prompt &optional min-point)
  "Read a Storypoint value with PROMPT from Fibonacci candidates.
When MIN-POINT is non-nil, offer only candidates greater than or equal to it."
  (string-to-number
   (my/org--completing-read-in-order
    prompt
    (mapcar #'number-to-string
            (seq-filter (lambda (point)
                          (or (null min-point) (>= point min-point)))
                        my/fibonacci-points)))))

(defun my/org--storypoint-estimate (optimistic most-likely pessimistic safety-factor)
  "Return PERT estimate using OPTIMISTIC, MOST-LIKELY, PESSIMISTIC and SAFETY-FACTOR."
  (let* ((expected (/ (+ optimistic (* 4 most-likely) pessimistic) 6.0))
         (sigma (/ (- pessimistic optimistic) 6.0)))
    (+ expected (* safety-factor sigma))))

(defun my/org-set-story-point ()
  "Prompt for three-point Storypoint estimates and set Org properties."
  (interactive)
  (let* ((optimistic (my/org--read-storypoint "Storypoint optimistic (O): "))
         (most-likely (my/org--read-storypoint "Storypoint most likely (M): " optimistic))
         (pessimistic (my/org--read-storypoint "Storypoint pessimistic (P): " most-likely)))
    (unless (<= optimistic most-likely pessimistic)
      (user-error "Expected O <= M <= P"))
    (let* ((safety-label
            (my/org--completing-read-in-order
             "Safety: "
             (mapcar #'car my/org-storypoint-safety-options)))
           (safety-factor (cdr (assoc safety-label my/org-storypoint-safety-options)))
           (expected (/ (+ optimistic (* 4 most-likely) pessimistic) 6.0))
           (sigma (/ (- pessimistic optimistic) 6.0))
           (selected (my/org--storypoint-estimate optimistic most-likely pessimistic safety-factor)))
      (org-set-property "StorypointOptimistic" (my/org--format-storypoint optimistic))
      (org-set-property "StorypointMostLikely" (my/org--format-storypoint most-likely))
      (org-set-property "StorypointPessimistic" (my/org--format-storypoint pessimistic))
      (org-set-property "StorypointExpected" (my/org--format-storypoint expected))
      (org-set-property "StorypointSigma" (my/org--format-storypoint sigma))
      (org-set-property "StorypointSafety" safety-label)
      (org-set-property "Storypoint" (my/org--format-storypoint selected)))))
    ...
    (defun my/org--get-effort-from-point (base-point base-minutes)
      ...
      (let ((total (round (* base-minutes base-point))))
  ...
  (let ((entries '())
    ...
    (if (null entries)
	  ...
	  (let* ((time-options my/org-storypoint-time-options)
	         (base-choice (my/org--completing-read-in-order
        ...
        (with-current-buffer (marker-buffer parent-marker)
          ...
          (let ((total (format "%d:%02d" (/ total-effort-minutes 60) (% total-effort-minutes 60))))
            ...
            (org-set-property "Storypoint" (my/org--format-storypoint total-storypoints))
            (message "Total effort: %s, Total storypoints: %s"
                     total
                     (my/org--format-storypoint total-storypoints))))))))
...
(defconst my/org-progress-weekend-options
  '(("include" . "土日を含める")
    ("exclude" . "土日を除外する"))
  "Weekend counting options for Storypoint progress planning.")

(defun my/org-set-progress-weekend ()
  "Set whether weekend days count in Storypoint progress planning."
  (interactive)
  (let ((choice (my/org--completing-read-in-order
                 "Progress weekend: "
                 (mapcar #'car my/org-progress-weekend-options))))
    (org-set-property "ProgressWeekend" choice)))

(defun my/org--timestamp-to-absolute-date (timestamp)
  "Return absolute date for Org TIMESTAMP, or nil when TIMESTAMP is missing."
  (when timestamp
...
(defun my/org--count-progress-days (start end include-weekend)
  "Count planning days from START to END, inclusive.
When INCLUDE-WEEKEND is nil, Saturdays and Sundays are skipped."
  (let ((count 0))
    (when (<= start end)
      (dotimes (offset (1+ (- end start)))
        (let ((day (+ start offset)))
          (when (or include-weekend
                    (not (my/org--weekend-day-p day)))
            (setq count (1+ count))))))
    count))

(defun my/org--progress-status-label (gap)
  "Return a Japanese progress status label for GAP Storypoints."
  (cond
   ((< gap -0.05) "遅れ")
   ((> gap 0.05) "前倒し")
   (t "順調")))

(defun my/org-progress-plan-summary (total-sp done-sp)
  "Return ideal Storypoint progress summary for TOTAL-SP and DONE-SP."
  (let* ((start (my/org--timestamp-to-absolute-date (org-entry-get nil "SCHEDULED")))
         (deadline (my/org--timestamp-to-absolute-date (org-entry-get nil "DEADLINE")))
         (weekend (or (org-entry-get nil "ProgressWeekend") "include"))
         (include-weekend (not (string= weekend "exclude"))))
    (when (and start deadline (> total-sp 0))
      (let* ((today (my/org--today-absolute-date))
             (total-days (my/org--count-progress-days start deadline include-weekend))
             (elapsed-end (min deadline today))
             (elapsed-days (if (< today start)
                               0
                             (my/org--count-progress-days start elapsed-end include-weekend))))
        (when (> total-days 0)
          (let* ((sp-per-day (/ total-sp (float total-days)))
                 (ideal-sp (min total-sp (* sp-per-day elapsed-days)))
                 (ideal-pct (* 100.0 (/ ideal-sp (float total-sp))))
                 (gap (- done-sp ideal-sp)))
            (format "STATUS: %s %+.1fSP  実績 %s/%sSP  今日の目安 %sSP  必要ペース %sSP/日  土日:%s"
                    (my/org--progress-status-label gap)
                    gap
                    (my/org--format-storypoint done-sp)
                    (my/org--format-storypoint total-sp)
                    (my/org--format-storypoint ideal-sp)
                    (my/org--format-storypoint sp-per-day)
                    (if include-weekend "含む" "除外"))))))))
...
(defun org-calc-progress ()
  ...
  (let* ((total-sp (string-to-number (or (org-entry-get nil "Storypoint") "0")))
         ...
         (eff-pct (if (> total-eff-min 0)
                    ...
                    0.0))
         (plan-summary (my/org-progress-plan-summary total-sp done-sp))
         (result
          (concat
           (when plan-summary
             (concat plan-summary "  "))
           (format "Storypoint: %.1f%% (%s/%s)"
                   sp-pct
                   (my/org--format-storypoint done-sp)
                   (my/org--format-storypoint total-sp))
           (format "  Effort: %.1f%% (%s/%s)"
                   eff-pct
...
(defun my/org-show-subtree-with-drawers-hidden ()
  "Show the current Org subtree while keeping drawers folded."
  (interactive)
  (org-back-to-heading t)
  (org-show-subtree)
  (save-excursion
    (let ((begin (save-excursion
          ...
          (end (save-excursion
                 ...
                 (point))))
      (my/org--hide-drawers-completely begin end))))

(defun my/org--hide-drawers-completely (begin end)
  "Hide drawer blocks completely between BEGIN and END."
  (save-excursion
    (goto-char begin)
    (while (and (< (point) end)
      ...
      (let* ((drawer (save-excursion
        ...
        (if (memq type '(drawer property-drawer))
            (let* ((drawer-end (min (org-element-end drawer) end))
                   (hide-end (save-excursion
                               (goto-char drawer-end)
                               ;; Keep the trailing newline visible so adjacent
                               ;; headings do not render on the same line.
                               (when (and (> (point) (org-element-begin drawer))
                                          (eq (char-before) ?\n))
                                 (backward-char))
                               (point))))
              (org-fold-region (org-element-begin drawer) hide-end t 'drawer)
              (goto-char drawer-end))
          (forward-line 1))))))

(defun my/org-cycle-hide-drawers-completely (state)
  "Hide drawer blocks completely after Org visibility cycle STATE."
  (when (and (derived-mode-p 'org-mode)
             (not (memq state '(overview folded contents))))
    (let* ((global-p (eq state 'all))
           (begin (if global-p
                      (point-min)
                    (save-excursion
                      (org-back-to-heading t)
...
(my/after-org-load
  (add-hook 'org-cycle-hook #'my/org-cycle-hide-drawers-completely)
  (define-key org-mode-map (kbd "C-c C-p") #'org-calc-progress)
  (define-key org-mode-map (kbd "C-c C-x C-g") #'my/org-set-progress-weekend)
  (define-key org-mode-map (kbd "C-c C-x C-t") #'my/org-show-subtree-with-drawers-hidden))
