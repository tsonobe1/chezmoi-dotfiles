...
** straight.el
  #+begin_src emacs-lisp
    ;; straight.el ブートストラップ（GitHub リポジトリからのパッケージ取得用）
    ;; Emacs 同梱の org-mode をそのまま使う（straight.el でビルドしない）
    ...
    ;; use-package で :straight キーワードを使えるようにする
...
#+begin_src emacs-lisp
  ;; org-storypoint パッケージ（GitHub: tsonobe1/org-storypoint）
  (use-package org-storypoint
    :straight (:host github :repo "tsonobe1/org-storypoint")
...
#+begin_src emacs-lisp
  ;; Drawer 非表示（org-storypoint とは独立した汎用機能）
